...
public class EducatorViewSubmitAssign {
...
@Test
public void testLoginAndRedirectToAssignments() {
    ...
    try {
        ...
        wait.until(ExpectedConditions.urlContains("/viewsubmitassignments"));
        String currentUrl = driver.getCurrentUrl();
        ...
        Assert.assertTrue("Login failed: Expected to be redirected to View Submitted Assignments page.",
                currentUrl.contains("/viewsubmitassignments"));
...
public void testViewCourses() {
    ...
    try {
        ...
        wait.until(ExpectedConditions.urlContains("/viewsubmitassignments"));
        String currentUrl = driver.getCurrentUrl();
        Assert.assertTrue("Login failed: Expected to be redirected to View Submitted Assignments page.", currentUrl.contains("/viewsubmitassignments"));
...
public void testGradeAssignment() {
    ...
    try {
        ...
        // Select an assignment for grading
        WebElement firstAssignmentCard = driver.findElement(By.xpath("//div[@class='assignment-card-view'][1]"));
        firstAssignmentCard.click();

        // Wait for grade input fields
        WebElement gradeInput = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='Enter grade']")));
        gradeInput.sendKeys("A");

        // Fill in feedback
        WebElement feedbackTextArea = driver.findElement(By.xpath("//textarea[@placeholder='Enter feedback']"));
        feedbackTextArea.sendKeys("Great job!");

        // Submit grade and feedback
        WebElement submitButton = driver.findElement(By.xpath("//button[contains(text(), 'Submit')]"));
        submitButton.click();

        // Wait for submission confirmation and verify the update
        Alert alert = wait.until(ExpectedConditions.alertIsPresent());
        Assert.assertEquals("Grade and feedback submitted successfully!", alert.getText());
        alert.accept();
...
public void testViewAssignments() {
    ...
    try {
        ...
        WebElement assignmentsList = wait.until(ExpectedConditions.visibilityOfElementLocated(By.className("assignments-list-view")));
        Assert.assertTrue("Assignments list not displayed.", assignmentsList.isDisplayed());

        // Check that assignments are listed
        WebElement firstAssignment = driver.findElement(By.xpath("//div[@class='assignment-card-view'][1]"));
        Assert.assertTrue("Assignment not found in the list.", firstAssignment.isDisplayed());
    } catch (Exception e) {
        e.printStackTrace();
    } finally {
        if (driver != null) {
...
@Test
public void testAssignmentWithoutFile() {
    driver.get("http://localhost:3000");
    ...
    try {
        ...
        // Select the first assignment without a file
        WebElement firstAssignment = driver.findElement(By.xpath("//div[@class='assignment-card-view'][1]"));
        WebElement fileLink = firstAssignment.findElement(By.xpath(".//a[contains(text(),'Download Assignment')]"));

        // Ensure the "No file uploaded" message is displayed
        Assert.assertTrue("No file uploaded message not found.", firstAssignment.getText().contains("No file uploaded"));
    } catch (Exception e) {
        e.printStackTrace();
    } finally {
        if (driver != null) {
...
// Add more tests for other functionalities as required (e.g., grading assignments, viewing courses, etc.)
